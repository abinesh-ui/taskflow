...
export default function MobileTaskView({ filterProjectId, filterDepartmentId }: MobileProps = {}) {
  ...
  const { data: departments = [] } = useQuery({ queryKey: ['departments'], queryFn: async () => { const { data } = await supabase.from('master_departments').select('*').eq('is_active', true).order('position'); return (data || []) as Array<Department & { color?: string }>; } });
