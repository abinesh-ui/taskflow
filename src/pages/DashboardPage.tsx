...
function TaskRow({ task, statuses, priorities, members, taskTypes, categories, taskSections, macroProjects, milestones, departments, projects, expanded, subtaskCount, onToggle, onUpdate, onAddSubtask, onCancel, onDelete, overdue, isSubtask, selected, onSelect }: any) {
  ...
  return (
    <tr className={`border-b hover:bg-accent/20 ${isSubtask ? 'bg-muted/10' : ''} ${overdue > 0 && !status?.is_closed ? 'bg-red-50/30 dark:bg-red-950/5' : ''} ${selected ? 'bg-primary/10' : ''}`}>
      ...
      <td className="py-1 px-0.5 text-[9px] text-muted-foreground truncate">{(() => { const p = projects?.find?.((pr:any)=>pr.id===task.project_id); return p ? <span className="px-1.5 py-0.5 rounded text-[9px] font-medium text-white" style={{backgroundColor: p.color || '#6b7280'}}>{p.name}</span> : ''; })()}</td>
      ...
      <td className="py-1 px-0.5">{(() => { const m = members.find((mb:any)=>mb.id===task.assignee_id); return <select value={task.assignee_id||''} onChange={(e) => onUpdate(task.id,'assignee_id',e.target.value)} className="text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium" style={{backgroundColor: m ? m.color+'20' : 'transparent', color: m?.color || 'inherit'}}><option value="">-</option>{members.map((mb:any)=><option key={mb.id} value={mb.id}>{mb.name}</option>)}</select>; })()}</td>
