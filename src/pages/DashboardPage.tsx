...
import { ChevronDown, ChevronRight, Plus, Download, ChevronsDown, ChevronsUp, ChevronLeft, Pencil, Check, X } from 'lucide-react';
...
export default function DashboardPage({ filterProjectId, filterDepartmentId }: DashboardProps = {}) {
  ...
  async function updateField(taskId: string, field: string, value: any) {
    ...
    queryClient.setQueryData(['all-tasks'], (old: any[]) =>
      (old || []).map((t: any) => t.id === taskId ? { ...t, [field]: value || null } : t)
    );
  }
...
function TaskRow({ task, statuses, priorities, members, taskTypes, categories, taskSections, macroProjects, milestones, departments, projects, projectMembers, expanded, subtaskCount, onToggle, onUpdate, onAddSubtask, onCancel, onDelete, overdue, isSubtask, selected, onSelect, getTaskCompletion, canDelete, canCancel }: any) {
  ...
  return (
    <tr className={`border-b hover:bg-accent/20 ${isSubtask ? 'bg-muted/10' : ''} ${overdue > 0 && !status?.is_closed ? 'bg-red-50/30 dark:bg-red-950/5' : ''} ${selected ? 'bg-primary/10' : ''}`}>
      ...
      <td className="py-1 px-0.5"><select value={editing ? ef.department_id||'' : task.department_id||''} onChange={(e) => editing ? setEf({...ef, department_id: e.target.value}) : onUpdate(task.id,'department_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${editing?'bg-blue-50 ring-1 ring-blue-200':''}`}>{departments.map((dp:any)=><option key={dp.id} value={dp.id}>{dp.name}</option>)}</select></td>
      <td className="py-1 px-0.5"><select value={editing ? ef.status_id : task.status_id} onChange={(e) => editing ? setEf({...ef, status_id: e.target.value}) : onUpdate(task.id,'status_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium text-white ${editing?'ring-1 ring-blue-200':''}`} style={{backgroundColor: status?.color || '#6b7280'}}>{statuses.map((s:any)=><option key={s.id} value={s.id}>{s.name}</option>)}</select></td>
      <td className="py-1 px-0.5"><select value={editing ? ef.priority_id||'' : task.priority_id||''} onChange={(e) => editing ? setEf({...ef, priority_id: e.target.value}) : onUpdate(task.id,'priority_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${editing?'bg-blue-50 ring-1 ring-blue-200':''}`}><option value="">-</option>{priorities.map((pr:any)=><option key={pr.id} value={pr.id}>{pr.name}</option>)}</select></td>
      <td className="py-1 px-0.5"><select value={editing ? ef.assignee_id||'' : task.assignee_id||''} onChange={(e) => editing ? setEf({...ef, assignee_id: e.target.value}) : onUpdate(task.id,'assignee_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${editing?'bg-blue-50 ring-1 ring-blue-200':''}`}><option value="">-</option>{filteredMembers.map((mb:any)=><option key={mb.id} value={mb.id}>{mb.name}</option>)}</select></td>
      <td className="py-1 px-0.5"><select value={editing ? ef.assigner_id||'' : (task as any).assigner_id||''} onChange={(e) => editing ? setEf({...ef, assigner_id: e.target.value}) : onUpdate(task.id,'assigner_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${editing?'bg-blue-50 ring-1 ring-blue-200':''}`}><option value="">-</option>{filteredMembers.map((mb:any)=><option key={mb.id} value={mb.id}>{mb.name}</option>)}</select></td>
      <td className="py-1 px-0.5"><select value={editing ? ef.task_type_id||'' : task.task_type_id||''} onChange={(e) => editing ? setEf({...ef, task_type_id: e.target.value}) : onUpdate(task.id,'task_type_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${editing?'bg-blue-50 ring-1 ring-blue-200':''}`}><option value="">-</option>{taskTypes.map((tt:any)=><option key={tt.id} value={tt.id}>{tt.name}</option>)}</select></td>
      <td className="py-1 px-0.5"><select value={editing ? ef.section_id||'' : (task as any).section_id||''} onChange={(e) => editing ? setEf({...ef, section_id: e.target.value}) : onUpdate(task.id,'section_id',e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${editing?'bg-blue-50 ring-1 ring-blue-200':''}`}><option value="">-</option>{taskSections.map((sc:any)=><option key={sc.id} value={sc.id}>{sc.name}</option>)}</select></td>
      ...
      <td className="py-1 px-0.5">
        <div className="flex items-center gap-0.5">
          ...
          {editing && <button onClick={saveEdit} className="h-4 w-4 flex items-center justify-center rounded bg-green-100 text-green-600" title="Save all"><Check className="h-3 w-3" /></button>}
          {editing && <button onClick={() => setEditing(false)} className="h-4 w-4 flex items-center justify-center rounded hover:bg-muted text-muted-foreground" title="Cancel edit"><X className="h-2.5 w-2.5" /></button>}
          {!editing && !isSubtask && <button onClick={onAddSubtask} className="h-4 w-4 flex items-center justify-center rounded hover:bg-primary/10 text-muted-foreground hover:text-primary" title="Add subtask"><Plus className="h-2.5 w-2.5" /></button>}
          {!editing && canCancel && <button onClick={onCancel} className="h-4 w-4 flex items-center justify-center rounded hover:bg-orange-100 text-muted-foreground hover:text-orange-600 text-[8px]" title="Cancel">✕</button>}
          {!editing && canDelete && <button onClick={onDelete} className="h-4 w-4 flex items-center justify-center rounded hover:bg-red-100 text-muted-foreground hover:text-red-600 text-[8px]" title="Delete">🗑</button>}
        </div>
      </td>
    </tr>
  );
