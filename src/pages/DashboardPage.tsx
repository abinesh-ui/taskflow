...
function TaskRow({ task, statuses, priorities, members, taskTypes, categories, taskSections, macroProjects, milestones, departments, projects, projectMembers, expanded, subtaskCount, onToggle, onUpdate, onAddSubtask, onCancel, onDelete, overdue, isSubtask, selected, onSelect, getTaskCompletion, canDelete, canCancel }: any) {
  ...
  return (
    <tr className={`border-b hover:bg-accent/20 ${editing ? 'bg-blue-50/40 dark:bg-blue-950/10' : isSubtask ? 'bg-muted/10' : ''} ${overdue > 0 && !status?.is_closed ? 'bg-red-50/30 dark:bg-red-950/5' : ''} ${selected ? 'bg-primary/10' : ''}`}>
      ...
      <td className="py-1 px-0.5">{(() => { const d = projDepartments.find((dp: any) => dp.id === (editing ? ef.department_id : task.department_id)); const val = editing ? ef.department_id : task.department_id; return <select value={val || ''} onChange={(e) => editing ? setEf({ ...ef, department_id: e.target.value }) : onUpdate(task.id, 'department_id', e.target.value)} className={`text-[9px] border-0 outline-none w-full rounded px-1 py-0.5 font-medium ${d?.color ? 'text-white' : 'text-foreground'}  ${editing ? 'ring-1 ring-blue-200' : ''}`} style={{ backgroundColor: d?.color ? (d.color) : 'transparent' }}><option value="">-</option>{projDepartments.map((dp: any) => <option key={dp.id} value={dp.id}>{dp.name}</option>)}</select>; })()}</td>
