          ...
          <tbody>
            ...
            {adding && (
              <tr className="border-b bg-primary/5">
                ...
                <td className="py-1.5 px-1"><select value={nf.department_id || ''} onChange={(e) => setNf({ ...nf, department_id: e.target.value })} className="h-6 text-[9px] w-full bg-transparent border-0 outline-none"><option value="">Dept</option>{(nf.project_id ? departments.filter((d) => d.project_id === nf.project_id) : departments).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></td>
            ...
            {filteredMilestones.map((ms) => {
              ...
              return (
                <tr key={ms.id} className="border-b hover:bg-accent/20">
                  ...
                  <td className="py-1.5 px-1"><select value={(ms as any).department_id || ''} onChange={(e) => updateField(ms.id, 'department_id', e.target.value)} className="text-[9px] bg-transparent border-0 outline-none w-full hover:bg-muted/50 rounded"><option value="">-</option>{departments.filter((d) => d.project_id === ms.project_id).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></td>
